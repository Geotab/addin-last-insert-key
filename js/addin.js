geotab.addin.lastInsertKey = function(api, state) {
    var container = document.getElementById("lastInsertKey-container"),
        ...
        applyOptions = function() {
            ...
            if (isValidDateString(fromDateString)) {
                ...
                alert("Invalid date! The correct format is \"mm/dd/yyyy\"");
            }
        },
        focus = function() {
            var date;
        ...
        loadedCount,
        lastInsertKeys,
        loadChanges = function(fromDate, drivers) {
            ...
            if (fromDate) {
                ...
                }).then(function (countOfChanges) {
                    loadPartOfChanges(null, countOfChanges, drivers);
                }, loadErrorHandler);
            }
        },
        loadPartOfChanges = function(fromDate, countOfChanges, drivers) {
            var to = new Date(loadEndDate),
                from,
                driverChangeSearch,
                driversHash = drivers.length === 0 ? null : drivers.reduce(function(driversHash, driverId) {
        ...
        filterTripsByDrivers = function(trips, driversHash) {
            if (!driversHash) {
                return trips;
            }
     ...
     * */
        processLoadedPart = function(trips) {
            var tasks = (function() {
                    var value = 0,
                        increase = function() {
                            value++;
                ...
                * */
                requestsControl = (function() {
                    var sentRequests = {},
                        DRIVER = "driver_",
                        DEVICE = "device_",
                        getEntityAPI = function(entityName) {
                            return {
                                sent: function(id) {
            ...
            trips.forEach(function(trip) {
                var driver,
                    driverId = trip.driver !== "UnknownDriverId" ? trip.driver.id : null,
                    device,
                    deviceId = trip.device ? trip.device.id : null;
        ...
        render = function(changes) {
            ...
            if (changes.length > 0) {
                for (i = 0; i < changes.length; i++) {
                    change = changes[i];
                    row = "<div class='lastInsertKey-row'>";
                ...
                data = "<div class='lastInsertKey-row'><div class='lastInsertKey-cell-name'>There is no driver changes in selected period.</div></div>";
            }
        ...
        driversInput = (function(container, searchHandler) {
            var searchContainer = document.createElement("input"),
                ...
                setOptionsState = function() {
                    Array.prototype.forEach.call(optionsContainer.childNodes, function(item) {
                ...
                isClickInsideArea = function(target) {
            ...
            //  1 - down
            // -1 - up
                moveHover = function(direction) {
                    var availableItems = [],
                        hoverItem = null;

                    Array.prototype.forEach.call(optionsContainer.childNodes, function(item, index) {
                    ...
                    Array.prototype.forEach.call(optionsContainer.childNodes, function(item, i) {
                        if (i === hoverItem) {
                            addClass(item, "hover");
                        } else {
        ...
        })(document.querySelector(".lastInsertKey-drivers"), function(keyword, callback) {
            var userSearch = {
                isDriver: true,
                driverGroups: state.getGroupFilter()
            };
        ...
        exportToXLSX = function() {
            var Workbook = function() {
                ...
                cellStyles = [
                    { applyFill: "1", applyBorder: "1", borderId: "0", fillId: "2", fontId: "1", numFmtId: 0, xfId: "0", applyAlignment: "1", alignment: alignLeft },
                    { applyFill: "1", applyBorder: "1", borderId: "0", fillId: "2", fontId: "2", numFmtId: 0, xfId: "0" },
                    { applyFill: "1", applyBorder: "1", borderId: "0", fillId: "2", fontId: "2", numFmtId: 0, xfId: "0", applyAlignment: "1", alignment: alignRight },
                    { applyFill: "1", applyBorder: "1", borderId: "0", fillId: "2", fontId: "1", numFmtId: "15", xfId: "0", applyAlignment: "1", alignment: alignLeft },
                    { applyFill: "1", applyBorder: "1", borderId: "0", fillId: "2", fontId: "1", numFmtId: "25", xfId: "0", alignment: alignLeft }
                ],
            ...
            // Created date
            worksheet.A1 = { v: "Created", s: 2 };
            worksheet.B1 = { v: new Date(), s: 3, t: "d" };

            // Total count
            worksheet.A2 = { v: "Total", s: 2 };
            worksheet.B2 = { v: "", f: "COUNTA(A5:A65536)", t: "n" };

            // Grid header
            worksheet.A4 = { v: "Driver", s: 1 };
            worksheet.B4 = { v: "Date", s: 1 };
            worksheet.C4 = { v: "Type", s: 1 };
            ...
            for (i = 0; i < changes.length; i++) {
                ...
                worksheet["A" + row] = { v: driverName };
                worksheet["B" + row] = { v: dateTime, s: 4, t: "d" };
                worksheet["C" + row] = { v: changeType };
            }
            ...
            saveAs(new Blob([s2ab(wbout)],{type:"application/octet-stream"}), "LastInsertKeyReport.xlsx");
        };
